...
**Status:** ⬜ open · 🟢 shipped (branch) · 🟡 in progress · ⏸️ deferred (documented, not done)
...
- **A1 🐞 — Plain-text bookmarks aren't HTML-escaped.** `BookmarkDto.toReaderArticle`
  ...
  newlines to `<br>`. **Status: 🟢 `claude/fix-plaintext-escaping`**
...
- **A2 🐞 — In-article anchor / footnote links open a browser.** The reader doc is
  ...
  (also re-anchors progress nicely). **Status: 🟢 `claude/reader-anchor-links`**

- **A3 🐞 — `StatsScreen` streak label has a dead conditional.**
  `if (days == 1) "day streak" else "day streak"` — both branches are identical,
  so "1 day streak" never becomes "1-day streak"/singular. A leftover that does
  nothing. **Status: 🟢 `claude/stats-polish`**
...
- **A4 🔧 — TTS init failure is invisible.** `ArticleSpeaker` sets
  ...
  nothing, with no feedback. Fix: surface a toast. **Status: 🟢 (folded into
  `claude/tts-polish`)**
...
- **B1 ✨ — Text-to-speech polish.** The "Listen" feature is great but:
  - doesn't request **audio focus**, so it talks over music/podcasts and won't
    duck or pause for calls/alarms;
  - doesn't pause when **headphones are unplugged** (becoming-noisy);
  - has **no speed control** (fixed 1.0×), which is the single most-requested TTS
    feature.
  Fix: AudioManager focus (pause on transient loss, stop on permanent loss),
  a becoming-noisy receiver, a persisted speech-rate cycled from the Listen bar,
  and the failure toast (A4). **Status: 🟢 `claude/tts-polish`**

- **B2 ✨/♿ — Reader can't pinch-zoom, and http images may be blocked.** Zoom is
  disabled (`builtInZoomControls = false`), so a small diagram or a wide table
  can't be enlarged — at odds with the care taken over legible fonts. And on an
  ...
  **Status: 🟢 `claude/reader-zoom-mixedcontent`**
...
- **D1 💡 — "Shuffle my typeface" 🎲.** A tiny dice in the reading-settings sheet
  ...
  Harmless, single-file, in keeping with "have fun." **Status: 🟢
  `claude/reader-typeface-shuffle`**
...
- **D2 💡 — "Finish by ~9:42pm" clock estimate** next to "12 min left" in the
  ...
  bar, which `claude/tts-polish` also edits — deferred to avoid a conflict).
...
Each item below is its own branch off `main`, with deliberately **disjoint file
sets** so the PRs merge without conflicting. `awesome.md` itself lives only on
`main` and is updated as branches land.

| Branch | What | Files |
|---|---|---|
| `claude/fix-plaintext-escaping` | A1: escape + paragraph-ify plain-text bodies | `data/remote/Mappers.kt` (+test) |
| `claude/reader-anchor-links` | A2: in-page anchors scroll instead of opening a browser | `reader/ReaderHtmlBuilder.kt` (+test) |
| `claude/tts-polish` | B1/A4: audio focus, headset-unplug, speech rate, failure toast | `tts/ArticleSpeaker.kt`, `ui/reader/ReaderViewModel.kt`, `ui/reader/ReaderScreen.kt`, `data/prefs/SettingsRepository.kt` |
| `claude/reader-zoom-mixedcontent` | B2: pinch-zoom + mixed-content compatibility | `ui/reader/ReaderWebView.kt` |
| `claude/stats-polish` | A3: streak pluralization fix | `ui/stats/StatsScreen.kt` |
| `claude/reader-typeface-shuffle` | D1: shuffle-typeface dice | `ui/reader/ReaderControls.kt` |

_Progress is updated on `main` as each branch is pushed._
